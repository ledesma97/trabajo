				...
				case Token.IDENTIFICADOR:
					...
					try{
						if((Arrays.asList("{","=",";","==",")").contains(Siguiente1Valor))) {
							...
							{
								tablasimbolos.add( new Identificador(to.getValor(), " ", "class"," ",to.getLinea()));
							}
						}
					...
					else if (to.getValor().equals(";"))
					{
						...
						try
						{
							if (Anterior2Tipo==Token.TIPO_DATO && Anterior1Tipo==Token.IDENTIFICADOR){
								tablasimbolos.add(new Identificador(Anterior1Valor,"",Anterior2Valor,"Global",to.getLinea()));
							}
							//VALIDAR LAS VARIABLES YA DECLARADAS.
								...
								if(Anterior4Tipo==Token.TIPO_DATO && x>0 && Anterior3Tipo==Token.IDENTIFICADOR){
									...
									impresion.add("Error semantico en linea "+to.getLinea()+ " la variable "+Anterior3Valor+" ya habia sido declarada en la linea "+tablasimbolos.get(auxRenglon).posicion);
									bandera=true;
								}

								if(!bandera)
									tablasimbolos.add(new Identificador(Anterior3Valor,Anterior1Valor,Anterior4Valor,"Global",to.getLinea()));
							}
								...
								ArrayList<String> expresion2 = new ArrayList<String>(expresion);
									...
									tablasimbolos.add(new Identificador(auxNombre,Resultadofinal+"",auxTipo,"Global",to.getLinea()));
	...
	public  Token AnalizadorSemantico (NodoDoble<Token> nodo){
		...
		if(nodo!=null) // si no llego al ultimo de la lista
		{
			...
			for (int i = 0; i < tablasimbolos.size(); i++) {
				...
				renglon = tablasimbolos.get(i).getPosicion();
				...
				if(aux.contains("int")){
					...
					if (EsNumeroEntero(aux2) == false && !aux2.isEmpty()) {
						banderaErroresSemanticos=true;
						impresion.add("Error Semantico en la linea "+renglon+ ", se recibi� un "+auxiliarTipo+ " y se esperaba un INT");
					} 
				}
	...
	public Token Semantico2(NodoDoble<Token> nodo) {
		...
		if(nodo!=null) // si no llego al ultimo de la lista
		{
			...
			if(to.getTipo()==Token.IDENTIFICADOR){
				...
				if(!bandera2){
					banderaErroresSemanticos=true;
					impresion.add("Error semantico en linea "+to.getLinea()+ " se uso la variable "+auxiliar+" no est� declarada");
				}
			}
